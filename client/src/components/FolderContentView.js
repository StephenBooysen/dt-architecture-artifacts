...
import { constructFileURL, constructFolderURL, getFolderName } from '../utils/urlUtils';
...
const FolderContentView = ({
  ...
  /**
   * Handles clicking on a file - navigates to the file URL and calls onFileSelect.
  ...
  /**
   * Handles clicking on a folder - navigates to the folder URL.
   * @param {Object} folder - The folder object
   */
  const handleFolderClick = (folder) => {
    const folderURL = constructFolderURL(currentSpace, folder.path);
    navigate(folderURL);
  };
  ...
  const handleBreadcrumbClick = (breadcrumbPath) => {
    if (breadcrumbPath === '') {
      ...
      // Navigate to folder
      const folderURL = constructFolderURL(currentSpace, breadcrumbPath);
      navigate(folderURL);
    }
  };
