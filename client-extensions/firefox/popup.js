/**
 * @fileoverview Firefox extension popup interface
 * 
 * Provides Firefox extension functionality for Design Artifacts including:
 ...
 * - Firefox-specific API utilization
 * 
...
class ArchitectureArtifactsExtension {
  ...
  async loadSettings() {
    try {
      const result = await browser.storage.sync.get(['serverUrl', 'currentSpace']);
      if (result.serverUrl) {
  ...
  async saveCurrentSpace() {
    try {
      await browser.storage.sync.set({ currentSpace: this.currentSpace });
    } catch (error) {
